...
#se muestra la figura final
    plt.show()
""" Acá se pone la instancia a correr """  
...
tenencia = 7  # tenencia tabu

vec = 6  # vecindarios para búsqueda tabu
...
print ("vec y tenencia",vec,tenencia)

#TiempoDeComputo = int(input ("Tiempo de ejecución: "))
TiempoDeComputo = 600 #tiempo usado en instancias finales
    ...
    if i==0:
        ...
        if Dimensiones[i][3]!=Dimensiones[i-1][3]:
            ...
            if SI[i-1][1]+Dimensiones[i-1][1]+Dimensiones[i][1]>MX[1]:      #para y
                ...
                if Dimensiones[i][5]*(0.5) < Dimensiones[i-1][5]:
                    SI[i][1]=SI[i-1][1]
                    maxy.append(Dimensiones[i][1])
                    maxy_n_c=maxy
                else:
...
while (True):
    ...
    for q in range(vec): #generación de varios vecinos
            ...
            for u in range(cm):
                    #generación de un vecino #se mueven varias cajas a la vez
                x=random.randrange(len(HA_ik))
                if x in Lista_tabu:
                    while x in Lista_tabu:
                        x=random.randrange(len(HA_ik))
                #asegurarse que no se repita la caja
                Lista_tabu.append(x)
    ...
    for k in range(len(Lista_listas)): #Se agregan todos las diferencias entre la función objetivo de cada vecindario 
                                       #y la de la sol actual HA_ik 
        mejor_orden.append(FO((np.array(Tabu[k])), Dimensiones, 0)-FO(HA_ik, Dimensiones, 0)) 
        
    n=0
    n=mejor_orden.index(min(mejor_orden))
    prueba=np.array(Tabu[n].copy())
    movv(prueba, Dimensiones)
    if FO(prueba, Dimensiones, 0) < min(MatrizOptima2):
        HA_ik=np.array(Tabu[n].copy())
        print("a")
        Listas_Tabu=Listas_Tabu+(Lista_listas[n])
        if (len(Listas_Tabu) == cm*tenencia):
            Listas_Tabu=Listas_Tabu[(cm-1)::]
        for i in range(cm): #Se añade el movimiento a memoria largo plazo
            Mem_largo_plazo[Lista_listas[n][i]-1]=Mem_largo_plazo[Lista_listas[n][i]-1]+1
        pass
                   
    else:        
        while True: #Se verifica que la mejor vecindad no tenga cajas tabú siendo movidas
            a=0
            b=0
            for k in Lista_listas[mejor_orden.index(min(mejor_orden))]:
                if k in Listas_Tabu:
                    b+=1
                    if b==cm:
                        mejor_orden[mejor_orden.index(min(mejor_orden))]=999999 #Si alguna caja es tabu, se actualiza el valor 
                        a=1  
                        break                 #
    ...
    if iteracion%p4==0:
        #Probar mover cierta cantidad de cajas con mayor frecuencia en Mem_largo_plazo
        #top_cuatro=[]
        mas_usados=[]
        lista=Mem_largo_plazo.copy()
        for x in range(cm): #seleccion de cajas
            #maximo = max(lista)
            a=lista.index(max(lista))# Buscamos el máximo valor de frecuencia 
            mas_usados.append(a)  # Lo añadimos a una nueva lista
            lista[a]=0
        #print(mas_usados)
        for i in range(5):  # Número de veces que ejecutamos este bucle
            Probar=HA_ik.copy()
            for j in range(cm): #movimiento de cajas
                movf(Probar ,Dimensiones, mas_usados[j])
            movv(Probar, Dimensiones)
            if FO(Probar, Dimensiones, 0) < FO(HA_ik, Dimensiones, 0): #Si se obtiene una mejor solución en cuanto al largo se actualiza sol actual
                HA_ik = Probar.copy()
...
"""        
    ...
    else:# si no se obtiene un optimo, se continua iterando con la solución anterior
