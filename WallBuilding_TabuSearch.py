...
#se muestra la figura final
    plt.show()
print("WallBuilding_Tabu")  
   
z="R08-2.csv"
Dimensiones=np.genfromtxt(z , dtype=int ,delimiter=',')
...
tenencia = 6 #tenencia tabú

vec = 4 #numero de vecindairos

Productos =len(Dimensiones) #N° de cajas
Clientes=max(Dimensiones[:,3]) #N° de cliente

#se muestra en consola
print ("tenencia y vecinos",tenencia, vec)

#TiempoDeComputo = int(input ("Tiempo de ejecución: "))
TiempoDeComputo = 29000 #tiempo usado en instancias finales
...
print("sol inicial tiempo: ", time()-starting_point)
...
while (True):
    ...
    for q in range(vec): #generación de varios vecinos
            ...
            for u in range(4):
                    #generación de un vecino #esto se hace moviendo varias cajas a la vez
    ...
    if iteracion%10==0:
        ...
        for i in range(10):
            ...
            if FO(Probar, Dimensiones, 0) < FO(HA_ik, Dimensiones, 0):
                HA_ik = Probar.copy() #Se prueban varias posibles cambios y en caso de superar la sol actual se actualiza
    ...
    else:# si no se obtiene un optimo, se continua iterando con la misma solucion
